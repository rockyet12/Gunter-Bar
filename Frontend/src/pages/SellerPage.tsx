...
import { BarChart3, Package, ShoppingCart, Users, TrendingUp, DollarSign, Clock, AlertTriangle } from 'lucide-react';
...
const SellerPage: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const activeCustomers = new Set(orders.map((order: any) => order.userId)).size;
      const pendingOrders = orders.filter((order: any) => order.status === 'pending').length;
      const lowStockItems = products.filter((product: any) => product.stock < 10).length;
